...
<style type="text/css">
   {
                ...
                <div class="container-fluid" >
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 id="title" class="h3 mb-0 text-gray-800">Referrals</h1>
                          ...
                          <div class="card-body card_"  >
                             ...
                             <div class="row">
                                 ...
                                 <div class="col-md-12">
                                    ...
                                    <button class="btn btn-light text-primary mb-4" style="font-size: 14px" onclick="window.location.href='all.php' ">All Referrals 
                                                                         <?php 
                                                                           $ciid =$_SESSION['gc_collegid'] ;
                                  ...
                                  <th scope="col">Email</th>
                                            ...
                                            <tbody>
                                             <?php 
                                                 ...
                                                 $getstudents = " select * from student where stud_college = '$ciid' and pds_filled = '1'  ";
                                                             $gstuds = mysqli_query($con,$getstudents); 
                                                           
                                                         
                                                              while($row = mysqli_fetch_array($gstuds)){
                                                                $student_email = $row['stud_email'];
                                                                   $studemail = substr($student_email, 0, strpos($student_email,'@'));
                                                                   $subj = $row['referral_subj'];
                                                                   $ids = $row['stud_id'];


                                                                       $getreferrals = "SELECT * FROM `referral_history` where stud_id = '$ids' and status = 0   ";
                                                                        $itemsfromreferral = mysqli_query($con,$getreferrals); 
                                                                           $countref= mysqli_num_rows($itemsfromreferral);
                                                                    while($history_ref = mysqli_fetch_array($itemsfromreferral)){
                                                                           $status = $history_ref['status']; 
                                                              ...
                                                              <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    if($subj == ''){
                                                                        ?>
                                                                           <button class="btn btn-light text-success ref " data-id="<?php echo $row['stud_id'] ?>" data-ln = "<?php echo $row['stud_lname'] ?>" data-fn="<?php echo $row['stud_fname'] ?>" data-mn ="<?php echo $row['stud_mname'] ?>" data-em="<?php echo $row['stud_email'] ?>" data-referral="<?php echo $row['referral_subj'] ?>" style="font-size: 12px">New Referral</button>
                                                                        <?php
                                                                    }else {
                                                                        ...
                                                                        //other stuffs
                                                                         ...
                                                                         if($countref >= 1){
                                                                              ?>
                                                                           <button class="btn btn-light text-warning ref " data-id="<?php echo $row['stud_id'] ?>" data-ln = "<?php echo $row['stud_lname'] ?>" data-fn="<?php echo $row['stud_fname'] ?>" data-mn ="<?php echo $row['stud_mname'] ?>" data-em="<?php echo $row['stud_email'] ?>" data-referral="<?php echo $row['referral_subj'] ?>" style="font-size: 12px">CONTINUE</button>
                                                                        <?php
                                                                         }else {
                                                                              ?>
                                                                           <button class="btn btn-light text-success ref " data-id="<?php echo $row['stud_id'] ?>" data-ln = "<?php echo $row['stud_lname'] ?>" data-fn="<?php echo $row['stud_fname'] ?>" data-mn ="<?php echo $row['stud_mname'] ?>" data-em="<?php echo $row['stud_email'] ?>" data-referral="<?php echo $row['referral_subj'] ?>" style="font-size: 12px">New Referral</button>
                                                                        <?php
                                                                             ...
                                                                             $countreferrals = "SELECT * FROM `referral_history` where stud_id ='$ids'  ";
                                                                              $allreferals = mysqli_query($con,$countreferrals); 
                                                                              $countingref= mysqli_num_rows($allreferals);
                                                                            if($countingref >=1){

                                                                                   while($rows = mysqli_fetch_array($allreferals)){
                                                        $stat = $rows['status'];
                                  ...
                                  if($coordinator == $token){
                                       ?>
                                                                     <button class="btn btn-light text-primary" onclick="window.location.href='referrals.php?tokenid=<?php echo $row['stud_id'] ?>&&ln=<?php echo $row['stud_lname'] ?>&&fn=<?php echo $row['stud_fname'] ?>' " style="font-size:13px"> View Referrals <span class="badge badge-danger">
                                                                        <?php echo $countingref ?>
                                                                            </span></button>
                                                                                <?php
                                    ...
                                    if(isset($_SESSION['show_rec'])){
                                      $show_ =  $_SESSION['show_rec'];
                                        ...
                                        if($show_ == 1){
                                             ...
                                             ?>
                                                                     <button class="btn btn-light text-secondary"  style="font-size:13px"> View Referrals </button>
                                                                                <?php
      ...
      <div class="modal-body">
        ...
        <form action="submit.php" method="post" id="saverefhistory" onsubmit="return false">
         ...
         <div class="container">
                ...
                <h6 style="font-weight:bolder;">New Referral</h6>
                ...
                <h6 class="mt-2">
                    ...
                    <input type="text" name="ref" id="ref" style="font-size:13px" class="form-control" required> <br>

                    Referred By : 

                    <input type="text" name="refby" id="refby" style="font-size:13px" class="form-control" required>
