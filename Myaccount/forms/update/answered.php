...
<?php 
	if($dd == 'file'){
		?>
								 ...
								 /*	if($secno >=1){
								 	...
								 	*/
								 	 ...
								 	 <div class="container mt-3">
								 	 		<span class="badge bg-danger" style="float:right;"> New </span>
					...
					<input type="file" name="<?php echo $formid ?>"   onchange="javascript:updateList<?php echo $formid  ?>()" id="file<?php echo $formid ?>" class="d-none" accept="image/*,.doc,.docs,.pdf,.ppt">
								 	 			 	 ...
								 	 			 	 $gettheattchfile = "SELECT * FROM `temp_answers` where formid = '$formid' and userid='$usertken'  ";
	...
	if($dd == 'Title') { 
			...
			$checkpds = "SELECT * FROM `temp_answers` where formid ='$fdlists' and tcontent_id='$tccontents' and userid='$usertoken' ";
										   			                 		$chpds = mysqli_query($con,$checkpds); 
										   			                 		 $countpdsa= mysqli_num_rows($chpds);
										   			                 		 if ($countpdsa>=1){
										   			                 		  ...
										   			                 		  while($pdsrespo = mysqli_fetch_array($chpds)){ 
										   			                 		  	?>
		<input type="text" name="" class="filllist"  data-tid= "<?php echo $pdsrespo['tid'] ?>"   style="width: 100%;font-size: 13px" placeholder="Fill this fields" value="<?php echo $pdsrespo['response'] ?>">
										   			                 		  	<?php
