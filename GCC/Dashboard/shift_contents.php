...
include '../create_form/connect.php';
                    						...
                    						<table class="table table-sm table-striped table-bordered" style="font-size:14px">
											 ...
											 <?php 
											 	...
											 	if(isset($_POST['yrlevel'])){
 													 	...
 													 	$get_studentdata = "select * from student where stud_id in (select stud_id from shifting_history where from_ = '$cid' ) and yearlevel ='$yrlevel' ";
 												}
	...
	//echo $year;
		          ...
		          $getallcolleges_ = "select * from colleges where CollegeId in (SELECT to_ FROM `shifting_history` where year(datecreated) = '$year' and semester = '$sem'  ) ";
                 $gettingcolleges = mysqli_query($con,$getallcolleges_); 
               	 $countfrom= mysqli_num_rows($gettingcolleges);
               	 if($countfrom >=1){
                      ...
                      text-align: center;
				        ...
				        <h6 style="font-size:16px;text-align:center;">Shifting To <?php echo $row['college'] ?></h6>
                    						...
                    						<table class="table table-sm table-striped table-bordered" style="font-size:14px">
											 ...
											 <?php 
										if(isset($_POST['yrlevel'])){
 													$get_studentdata = "select * from student where stud_id in (select stud_id from shifting_history where to_ = '$cid' )";
 										}else {
 												$get_studentdata = "select * from student where stud_id in (select stud_id from shifting_history where to_ = '$cid' ) and yearlevel = '$yrlevel'";
 										}	 
											 	 $result = mysqli_query($con,$get_studentdata); 
											 	 $countdata= mysqli_num_rows($result);
